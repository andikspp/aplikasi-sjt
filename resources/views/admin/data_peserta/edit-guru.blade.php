                        ...
                        <form action="{{ route('admin.update.guru', $guru->id) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control @error('jenis_paud') is-invalid @enderror" id="jenis_paud"
                                    ...
                                    <option value="mitra"
                                        {{ old('jenis_paud', $guru->jenis_paud) == 'mitra' ? 'selected' : '' }}>Mitra
                                    </option>
                                    <option value="pembelajar"
                                        {{ old('jenis_paud', $guru->jenis_paud) == 'pembelajar' ? 'selected' : '' }}>
                                        Pembelajar</option>
                                </select>
                                @error('jenis_paud')
                                    <div class="invalid-feedback">{{ $message }}</div>
    ...
    <script>
        ...
        function updatePaketSoal(role) {
            ...
            for (const [id, name] of Object.entries(paket)) {
                let selected = "{{ old('question_set_id', $guru->question_set_id) }}" == id ? 'selected' : '';
